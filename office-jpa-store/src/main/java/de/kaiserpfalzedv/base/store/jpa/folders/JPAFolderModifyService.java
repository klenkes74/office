...
import de.kaiserpfalzedv.base.store.jpa.folders.converters.FolderModifiedConverter;
import de.kaiserpfalzedv.base.store.jpa.folders.converters.ModifyFolderConverter;
...
public class JPAFolderModifyService implements FolderCommandService<ModifyFolder> {
    ...
    public void observe(@Observes final ModifyFolder command) {
        ...
        if (JPAFolder.find("scope = ?1 and key = ?2", spec.getIdentity().getTenant().orElse("./."), spec.getIdentity().getName()).count() != 0) {
