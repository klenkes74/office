...
public class FolderCommandServiceTest {
    ...
    public void shouldReturnValidDataWhenLoadingById() {
        Folder result = service.load(ID).orElseThrow(IllegalStateException::new);
    ...
    @Test
    public void shouldReturnEmptyOptionalWhenGivenRandomUUID() {
        Optional<Folder> result = service.load(UUID.randomUUID());
        LOGGER.trace("Loaded: {}", result);
